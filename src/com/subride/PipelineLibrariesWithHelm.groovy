...
class PipelineLibrariesWithHelm implements Serializable {

    def script
    def envVars = [:]

    PipelineLibrariesWithHelm(script) {
        this.script = script
    }
    ...
    def buildAndDeploy() {
        def containers = [
            ...
            script.containerTemplate(name: 'helm', image: 'alpine/helm:3.7.0', ttyEnabled: true, command: 'cat'), 
    ...
    def deploy() {
        script.container("helm") {
            script.withCredentials([script.usernamePassword(
                ...
                // Add Helm repo
                script.sh "helm repo add subride https://cna-bootcamp.github.io/helm-charts/stable --username ${HELM_USERNAME} --password ${HELM_PASSWORD}"
